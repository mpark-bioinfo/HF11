...
tiff("Plasma_Ctrl_all.tiff",    width = 5*300,        # 5 x 300 pixels
     height = 5*300,
     res = 300,            # 300 pixels per inch
     pointsize = 8)        # smaller font size)

heatmap.2(ctrl.ordered.df,
          main = "Control groups", # heat map title
...
#tiff(paste(args[2], filename1,'_heatmap_all.tiff',sep = ""),
tiff("Plasma_Case_all.tiff",    width = 5*300,        # 5 x 300 pixels
     height = 5*300,
     res = 300,            # 300 pixels per inch
     pointsize = 8)        # smaller font size)

heatmap.2(case.ordered.df,
          main = "Case groups", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
